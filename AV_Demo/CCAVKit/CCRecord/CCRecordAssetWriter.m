...
- (void)startRecording {
    ...
    self.status = CCRecordStatusRecording;
    if (self.startRecordBlock) self.startRecordBlock(self.currentURL);
}
...
- (void)stopRecording {
    ...
    if (url && ![self.videoArr containsObject:url]) {
        [self.videoArr addObject:url];
        NSLog(@"add new url file: %@",self.videoArr);
    }
    [self stopWrite];
}
...
- (void)initializedAssetWriter {
    NSLog(@"新建AssetWriter对象");
    
    [self destoryWriter];
    self.currentURL = [self availableFileURLToTmp];
    unlink([self.currentURL.path UTF8String]);
    NSLog(@"获取可用的临时路径: %@",self.currentURL.path);
...
- (void)stopWrite {
    ...
    }else if (self.status == CCRecordStatusFinish) {
...
- (AVAssetWriter *)assetWriter {
    if (!_assetWriter) {
        NSError *error = nil;
        NSURL *fileURL = self.currentURL;
        if (fileURL == nil) {
            fileURL = [self availableFileURLToTmp];
        }
