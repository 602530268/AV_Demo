...
- (instancetype)initWithURL:(NSURL *)url containerView:(UIView *)containerView {
    if (self = [super init]) {
        ...
        if (_fileURL == nil) {
            _fileURL = [self availableFileURLToSave];
        }
    }
    return self;
}
...
- (void)finishRecording {
    ...
    NSLog(@"fileURL: %@",self.fileURL.path);
...
- (NSURL *)availableFileURLToSave {
    ...
    NSString *tmpFileName = @"video_";
    NSUInteger now = (NSUInteger)([[NSDate date] timeIntervalSince1970] * 1000);
    NSString *path = [NSString stringWithFormat:@"%@/%@%lu.mp4",filePath,tmpFileName,now];
    return [NSURL fileURLWithPath:path];
}

- (NSURL *)availableFileURLToTmp {
    NSString *filePath = [NSString stringWithFormat:@"%@/%@",NSHomeDirectory(),kTmpPath];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:filePath]) {
        [fileManager createDirectoryAtPath:filePath withIntermediateDirectories:YES attributes:nil error:nil];
    }
    NSString *path = [NSString stringWithFormat:@"%@/video_tmp_%lu.mp4",filePath,(unsigned long)self.videoArr.count];
...
- (void)cancelRecord {
    ...
    NSString *path = [NSString stringWithFormat:@"%@/%@",NSHomeDirectory(),kTmpPath];
    NSError *error = nil;
    NSFileManager *fileManager = [NSFileManager defaultManager];
